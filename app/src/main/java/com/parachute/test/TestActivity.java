        ...
        public void run() {
            if (!IsTg) {//如果此时是静负荷测试
                ...
                if (MyApp.wy3Connected) {
                    ...
                    if (!"".equals(length2.trim()) && length2.matches("[0-9]*\\.{1}\\d*")) {
...
//        testFragment.refreshNull();
    ...
    public boolean SaveData() {
        ...
        try {
            ...
            if (IsTg) {//脱钩保存
                ...
                mres.setZdjsd(mJsd);// 制动加速度(最大加速度)
    ...
    private void onReceivedSensorData() {
        ComA.setOnReceivedSensorListener(new SerialControl.OnReceivedSensorData() {
            ...
            @Override
            public void draw(final float endKxc, final float zdsj) {//画图
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
...
//                        mJsdBuffer = ComA.mJsdBuffer;
    ...
    private void Draw() {
        ...
        // 最大加速度(写死，值为-99上下0.5浮动)
        MaxJsd = -99f + ((float) Math.random() - 0.5f);
        //mJsdBuffer,是list数组，里面存放byte[]数组
        //mJsdBuffer长度60，每一个byte[]长度99
...
//            }
    ...
    public void fenxi(float EndKxc, float zdsj) {
        DecimalFormat df2 = new DecimalFormat("0.00");
...
//            EndKxc = (int) (MaxIndex - zdsj / 4);
            ...
            mKxcsj = (float) ((EndKxc) * 0.25 / 1000f);
            //mXjgd：下降高度？
            //indexXjgd：6
            ...
            Log.i("weee", "mmaxLength（下降高度）: " + mmaxLength);
            while (((0.5 * stG * mKxcsj * mKxcsj * 1000f) > mmaxLength * 0.9) && (mmaxLength != 0.0f)) {
...
//                SUMJSD = SUMJSD + Math.abs(mJsdList[i]);
            ...
            mKxcsj = (float) ((EndKxc) * 0.25 / 1000f);//空行程时间
            mZdsj = (float) (zdsj / 1000f);//制动时间？
            mKxcjl = (float) (0.5 * stG * mKxcsj * mKxcsj * 1000f);//空行程距离
